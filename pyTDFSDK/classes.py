...
class TsfSpectrum(object):
    ...
    def get_lcms_tsf_data(self):
        frames_dict = self.tsf_data.analysis["Frames"][
            self.tsf_data.analysis["Frames"]["Id"] == self.frame
        ...
        self.mz_array, self.intensity_array = extract_tsf_spectrum(
            self.tsf_data,
            self.frame,
            self.mode,
            self.profile_bins,
        ...
        if (
            self.mz_array is not None
            and self.intensity_array is not None
            and self.mz_array.size != 0
            and self.intensity_array.size != 0
            and self.mz_array.size == self.intensity_array.size
        ):
            self.total_ion_current = sum(self.intensity_array)
            base_peak_index = np.where(
                self.intensity_array == np.max(self.intensity_array)
            )
            self.base_peak_mz = self.mz_array[base_peak_index][0].astype(float)
            self.base_peak_intensity = self.intensity_array[base_peak_index][0].astype(
                float
            )
            ...
            if int(frames_dict["MsMsType"]) == 0:
                ...
                framemsmsinfo_dict = self.tsf_data.analysis["FrameMsMsInfo"][
                    self.tsf_data.analysis["FrameMsMsInfo"]["Frame"] == self.frame
                ].to_dict(orient="records")[0]
                # Auto MS/MS
                if int(frames_dict["ScanMode"]) == 1:
                    ...
                    self.isolation_lower_offset = (
                        float(framemsmsinfo_dict["IsolationWidth"]) / 2
                    )
                    self.isolation_upper_offset = (
                        float(framemsmsinfo_dict["IsolationWidth"]) / 2
                    )
    ...
    def get_maldi_tsf_data(self):
        ...
        maldiframeinfo_dict = self.tsf_data.analysis["MaldiFrameInfo"][
            self.tsf_data.analysis["MaldiFrameInfo"]["Frame"] == self.frame
        ].to_dict(orient="records")[0]
        ...
        if (
            ...
            # MS/MS
                ...
                msms_mode_id = self.tsf_data.analysis["PropertyDefinitions"][
                    (
                        self.tsf_data.analysis["PropertyDefinitions"]["PermanentName"]
                        == "Mode_ScanMode"
                    )
                ].to_dict(orient="records")[0]["Id"]
                msms_mode = self.tsf_data.analysis["Properties"][
                    (self.tsf_data.analysis["Properties"]["Frame"] == self.frame)
                    & (self.tsf_data.analysis["Properties"]["Property"] == msms_mode_id)
...
class TdfSpectrum(object):
    ...
    def get_lcms_tdf_data(self):
        frames_dict = self.tdf_data.analysis["Frames"][
            self.tdf_data.analysis["Frames"]["Id"] == self.frame
        ].to_dict(orient="records")[0]
        self.polarity = frames_dict["Polarity"]
        ...
        if int(frames_dict["MsMsType"]) == 0:
            ...
            if not self.exclude_mobility:
                self.mz_array, self.intensity_array, self.mobility_array = (
                    extract_3d_tdf_spectrum(
                        self.tdf_data, self.frame, 0, int(frames_dict["NumScans"])
                    )
                )
            elif self.exclude_mobility:
                self.mz_array, self.intensity_array = extract_2d_tdf_spectrum(
                    self.tdf_data,
                    self.frame,
                    0,
            ...
            if (
                ...
                self.base_peak_mz = self.mz_array[base_peak_index][0].astype(float)
                self.base_peak_intensity = self.intensity_array[base_peak_index][
                    0
                ].astype(float)
                self.high_mz = float(max(self.mz_array))
                self.low_mz = float(min(self.mz_array))
        # diaPASEF
            ...
            if self.diapasef_window is not None:
                ...
                if (
                    ...
                    self.isolation_lower_offset = (
                        float(self.diapasef_window["IsolationWidth"]) / 2
                    )
                    self.isolation_upper_offset = (
                        float(self.diapasef_window["IsolationWidth"]) / 2
                    )
                ...
                raise Exception(
                    "diapasef_window from TdfData.analysis['DiaFrameMsMsWindows'] table required to parse "
                    "diaPASEF data."
                )
        # bbCID
        elif int(frames_dict["ScanMode"]) == 4 and int(frames_dict["MsMsType"]) == 2:
            framemsmsinfo_dict = self.tdf_data.analysis["FrameMsMsInfo"][
                self.tdf_data.analysis["FrameMsMsInfo"]["Frame"] == self.frame
            ].to_dict(orient="records")[0]
            if not self.exclude_mobility:
                self.mz_array, self.intensity_array, self.mobility_array = (
                    extract_3d_tdf_spectrum(
        ...
        # prmPASEF
            ...
            prmframemsmsinfo_dict = self.tdf_data.analysis["PrmFrameMsMsInfo"][
                self.tdf_data.analysis["PrmFrameMsMsInfo"]["Frame"] == self.frame
            ].to_dict(orient="records")[0]
            prmtargets_dict = self.tdf_data.analysis["PrmTargets"][
                self.tdf_data.analysis["PrmTargets"]["Id"]
                == int(prmframemsmsinfo_dict["Target"])
            ].to_dict(orient="records")[0]
            self.mz_array, self.intensity_array = extract_2d_tdf_spectrum(
                self.tdf_data,
                self.frame,
            ...
            if (
                ...
                self.isolation_lower_offset = (
                    float(prmframemsmsinfo_dict["IsolationWidth"]) / 2
                )
                self.isolation_upper_offset = (
                    float(prmframemsmsinfo_dict["IsolationWidth"]) / 2
                )
                ...
                if not np.isnan(prmtargets_dict["Charge"]):
                    self.selected_ion_ccs = tims_oneoverk0_to_ccs_for_mz(
                        self.tdf_data.api,
    ...
    def get_ddapasef_precursor_data(self):
        precursor_dict = self.tdf_data.analysis["Precursors"][
            self.tdf_data.analysis["Precursors"]["Id"] == self.precursor
        ].to_dict(orient="records")[0]
        frames_dict = self.tdf_data.analysis["Frames"][
            self.tdf_data.analysis["Frames"]["Id"] == precursor_dict["Parent"]
        ].to_dict(orient="records")[0]
        pasefframemsmsinfo_dicts = self.tdf_data.analysis["PasefFrameMsMsInfo"][
            self.tdf_data.analysis["PasefFrameMsMsInfo"]["Precursor"] == self.precursor
        ].to_dict(orient="records")
        self.polarity = frames_dict["Polarity"]
        self.centroided = get_centroid_status(self.mode, self.exclude_mobility)[0]
        self.retention_time = float(frames_dict["Time"]) / 60
        self.mz_array, self.intensity_array = extract_ddapasef_precursor_spectrum(
            self.tdf_data,
        ...
        if (
            ...
            self.isolation_lower_offset = (
                float(pasefframemsmsinfo_dicts[0]["IsolationWidth"]) / 2
            )
            self.isolation_upper_offset = (
                float(pasefframemsmsinfo_dicts[0]["IsolationWidth"]) / 2
            )
            ...
            self.selected_ion_mobility = tims_scannum_to_oneoverk0(
                self.tdf_data.api,
    ...
    def get_maldi_tdf_data(self):
        ...
        maldiframeinfo_dict = self.tdf_data.analysis["MaldiFrameInfo"][
            self.tdf_data.analysis["MaldiFrameInfo"]["Frame"] == self.frame
        ].to_dict(orient="records")[0]
        ...
        if int(frames_dict["MsMsType"]) == 0:
            ...
            msms_mode_id = self.tdf_data.analysis["PropertyDefinitions"][
                self.tdf_data.analysis["PropertyDefinitions"]["PermanentName"]
                == "Mode_ScanMode"
            ].to_dict(orient="records")[0]["Id"]
            msms_mode = self.tdf_data.analysis["Properties"][
                (self.tdf_data.analysis["Properties"]["Frame"] == self.frame)
                & (self.tdf_data.analysis["Properties"]["Property"] == msms_mode_id)
            ].to_dict(orient="records")[0]["Value"]
            # MALDI MS/MS, coded as MRM in the schema
            if msms_mode == 3:
                framemsmsinfo_dict = self.tdf_data.analysis["FrameMsMsInfo"][
                    self.tdf_data.analysis["FrameMsMsInfo"]["Frame"]
                    == maldiframeinfo_dict["Frame"]
                ].to_dict(orient="records")[0]
                if not self.exclude_mobility:
                    self.mz_array, self.intensity_array, self.mobility_array = (
                        extract_3d_tdf_spectrum(
            ...
            # MALDI prmPASEF
                ...
                if self.diapasef_window is not None:
                    ...
                    if (
                        ...
                        self.selected_ion_mz = float(
                            self.diapasef_window["IsolationMz"]
                        )
                        self.collision_energy = self.diapasef_window["CollisionEnergy"]
                        self.total_ion_current = sum(self.intensity_array)
                        base_peak_index = np.where(
                        ...
                        self.base_peak_mz = self.mz_array[base_peak_index][0].astype(
                            float
                        )
                        self.base_peak_intensity = self.intensity_array[
                            base_peak_index
                        ][0].astype(float)
                        self.high_mz = float(max(self.mz_array))
                        self.low_mz = float(min(self.mz_array))
                    ...
                    raise Exception(
                        "diapasef_window from TdfData.analysis['DiaFrameMsMsWindows'] table required to "
                        "parse diaPASEF data."
                    )
